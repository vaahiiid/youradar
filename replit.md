...
- **Sign-in / sign-up:** Custom branded screens at `app/(auth)/sign-in.tsx` and `app/(auth)/sign-up.tsx` (native Clerk components are incompatible with Expo Go). Both screens support **email + password** (with email verification code on sign-up) via `useSignIn` / `useSignUp`, and **Continue with Google** SSO via `useSSO` + `expo-auth-session`. Styling uses the YourRadar teal/charcoal brand palette.
