...
- **Browser flow** — On native, `WebBrowser.openAuthSessionAsync(authorizeUrl, Linking.createURL("oauth-success"))` runs the OAuth dance in a system-managed in-app browser, intercepts the deep-link callback, and the app navigates to `/oauth-success` with parsed params. On web, the sheet calls `window.open(authorizeUrl, "_blank", "noopener,noreferrer,width=560,height=720")` to break out of the iframe (Google and most providers refuse OAuth inside iframes via `disallowed_useragent`/`X-Frame-Options`, which surfaces as a 403 "you do not have access to this page"). If `window.open` is blocked, the sheet attempts `window.top.location.href` as a fallback, then renders a manual escape hatch with three pieces of UI: a primary **"Open {Provider} connection in browser"** button, a **"Copy link"** button (uses `navigator.clipboard.writeText`), and the full authorize URL displayed in a selectable monospace box. The URL is also `console.log`-ed for manual testing. After the popup completes the OAuth flow, the popup's `/oauth-success` route detects `window.opener`, posts a `{ type: "yourradar:oauth-result", payload: {...} }` message back to the parent app (which is listening in `ConnectAccountSheet`), and auto-closes after 1.2s; the parent then navigates the main app to `/oauth-success` so the user lands on the success screen inside the main UI.
